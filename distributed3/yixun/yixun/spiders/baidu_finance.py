    ...
    def parse_base(self, response):
        ...
        if 'http://finance.china.com.cn/news/' in response.url:
            ...
            try:
                ...
                item['content'] = response.xpath('//div[@id="Cnt-Main-Article-QQ"]/p[@style]').xpath('string()').extract()
            ...
            item['title'] = response.xpath('//div[@class="left"]/div[@class="titmain"]/h1/text()').extract()[2]
            item['time'] = response.xpath('//div[@class="titmain"]/p/span[1]/text()').extract()[1].strip().replace('/', '-').replace('年', '-').replace('月', '-').replace('日', '')
            item['source'] = response.xpath('//div[@class="titmain"]/p/span[2]/text()').extract()[1]
            item['content'] = response.xpath('//div[@id="container-article" or @class="texttit_m1"]').xpath('string()').extract()[0].replace('\t', '').replace('\n', '').replace(' ', '').replace('\r', '')
            item['author'] = response.xpath('//span[@id="author_baidu"]/a/text()').extract_first() or ''
            item['edit'] = response.xpath('//div[@class="time-source"]/span/a/text()').extract_first() or ''
            yield item
